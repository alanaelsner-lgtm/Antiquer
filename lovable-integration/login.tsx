...
import { authPlayer } from "@/lib/serverSync";
...
function LoginPage() {
  ...
  const onName = async (e: React.FormEvent) => {
    e.preventDefault();
    ...
    const player = await authPlayer(trimmed, trimmedPin, isNew);
    if (!player) {
      setError(isNew ? "That name is taken — try another!" : "Wrong name or PIN. Try again!");
      setLoading(false);
      return;
    }

    // Store credentials for syncing
    try {
      localStorage.setItem("antiquer:player", trimmed);
      localStorage.setItem("antiquer:playerId", player.id);
    } catch {}

    // If returning player has a character saved, go straight to play
    if (!isNew && player.character) {
      const found = CHARACTERS.find((c) => c.id === player.character);
      if (found) {
        try { localStorage.setItem("antiquer:character", found.id); } catch {}
        navigate({ to: "/play" });
        return;
      }
    }
      ...
      <div className="relative z-10 flex h-screen flex-col items-center justify-center px-4 py-6">
        ...
        >
          ...
          <div className="relative rounded-[28px] border-[5px] border-black bg-[oklch(0.97_0.04_85)] px-6 py-4 shadow-[0_10px_0_#000] sm:px-12 sm:py-6">
            ...
            <p className="mt-1 font-display text-2xl font-extrabold tracking-[0.15em] text-black sm:text-4xl">
              FLEA MARKET
            </p>
          </div>
        </motion.div>
            ...
            {step === "name" ? (
              ...
              >
                <h1 className="font-display text-3xl font-extrabold text-black sm:text-4xl">
                  {isNew ? "New antiquer!" : "Welcome back!"}
                </h1>
                <p className="mt-1 text-sm font-semibold text-black/70">
                  {isNew ? "Pick a name and a 4-digit PIN." : "Enter your name and PIN."}
                </p>

                {error && (
                  <p className="mt-3 rounded-xl bg-red-100 px-3 py-2 text-sm font-bold text-red-700">
                    {error}
                  </p>
                )}
                ...
                <button
                  ...
                  className="mt-3 w-full text-center text-xs font-bold uppercase tracking-[0.2em] text-black/50 hover:text-black/80"
                >
                  {isNew ? "Already have an account? Sign in" : "New here? Sign up"}
                </button>

                <p className="mt-2 text-center text-[11px] font-bold uppercase tracking-[0.2em] text-black/40">
                  No email needed — saves across devices
                </p>
              </motion.form>
            ) : (
              <motion.div
